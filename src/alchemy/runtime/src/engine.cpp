...
TRTEngine::TRTEngine(const std::string &plan_path) {
  ...
  int max_batch =
      engine_
          ->getProfileShape(input_name, 0, nvinfer1::OptProfileSelector::kMAX)
          .d[0];
  ...
  for (int i = 0; i < engine_->getNbIOTensors(); ++i) {
    ...
    // Timesteps are int64 so we need to adjust buffer allocation
    // TODO: replace this with the dtype map lookup when implemented
    if (engine_->getTensorDataType(name) == nvinfer1::DataType::kINT64)
      elem_size = 8;

    CUDA_CHECK(cudaMalloc(&buf, max_batch * n * elem_size));
...
TRTEngine::~TRTEngine() {
  for (auto &[name, ptr] : device_buffers_)
    cudaFree(ptr);
...
void TRTEngine::run(
    ...
    int batch_size) {
  /* TODO: refactor to support arbitrary dtypes (currently only supports float)
  possibly via a map lookup (tensor_name, dtype) and then switch *sizeof
  based on that.
  */
  ...
  for (auto &[name, data] : inputs) {
    ...
    CUDA_CHECK(
        cudaMemcpyAsync(device_buffers_.at(name), data.data(),
  ...
  // Can we remove this? Otherwise blocking on each step (hot path)
